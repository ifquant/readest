...
pub fn build_dyn_runner_from_spec(spec: &MarketDataSpec) -> Option<DynTickAggregatorRunner> {
    match spec {
        ...
        MarketDataSpec::Timeframe(tf) => Some(DynTickAggregatorRunner::new(Box::new(TimeframeBarAgg::new(tf.clone())))),
        MarketDataSpec::Volume(v) => Some(DynTickAggregatorRunner::new(Box::new(VolumeBarAgg::new(v.clone())))),
        MarketDataSpec::TickCount(tc) => Some(DynTickAggregatorRunner::new(Box::new(TickCountAgg::new(tc.clone())))),
        MarketDataSpec::Range(r) => Some(DynTickAggregatorRunner::new(Box::new(RangeBarAgg::new(r.clone())))),
        MarketDataSpec::Renko(rk) => Some(DynTickAggregatorRunner::new(Box::new(RenkoBrickAgg::new(rk.clone())))),

        // 成交额、微结构与加权
        MarketDataSpec::Dollar(d) => Some(DynTickAggregatorRunner::new(Box::new(DollarBarAgg::new(d.clone())))),
        MarketDataSpec::Imbalance(im) => Some(DynTickAggregatorRunner::new(Box::new(ImbalanceAgg::new(im.clone())))),
        MarketDataSpec::Delta(ds) => Some(DynTickAggregatorRunner::new(Box::new(DeltaAgg::new(ds.clone())))),
        MarketDataSpec::Vwap(vw) => Some(DynTickAggregatorRunner::new(Box::new(VwapAgg::new(vw.clone())))),

        // 价格分布影像
        MarketDataSpec::Footprint(fp) => Some(DynTickAggregatorRunner::new(Box::new(FootprintAgg::new(fp.clone())))),
        MarketDataSpec::VolumeProfile(vp) => Some(DynTickAggregatorRunner::new(Box::new(VolumeProfileAgg::new(vp.clone())))),
        MarketDataSpec::Tpo(tpo) => Some(DynTickAggregatorRunner::new(Box::new(TpoAgg::new(tpo.clone())))),

        // 线型/反转类
        MarketDataSpec::Kagi(kg) => Some(DynTickAggregatorRunner::new(Box::new(KagiAgg::new(kg.clone())))),
        MarketDataSpec::ThreeLineBreak(tlb) => Some(DynTickAggregatorRunner::new(Box::new(ThreeLineBreakAgg::new(tlb.clone())))),

        // 点数图（PNF）
        MarketDataSpec::Pnf(pnf) => Some(DynTickAggregatorRunner::new(Box::new(PnfAgg::new(pnf.clone())))),
    }
}

...
impl TickDerived for PnfAgg {
    ...
    fn on_tick_into(&mut self, tick: &Tick, out: &mut Vec<Self::Output>) {
        ...
        // 初始化：等待首个方向形成至少一个 box
        if self.current_mark.is_none() {
            // 以当前价为锚，若上涨/下跌超过一个 box 则建立首列
            let up_steps = (tick.price / box_size).floor() as i64 - ((tick.price - box_size) / box_size).floor() as i64; // 始终为 1
            // 更稳妥的初始判断：尝试从当前价向上/向下各检查一步
            let anchor = tick.price;
            if anchor >= (anchor - box_size) + box_size { // 始终成立，改为显式比较
                // 若下一笔价格继续上涨超过一个 box
            }
            // 使用简单策略：后续 tick 将推动形成列，这里不做提前创建
        }
        ...
        match self.current_mark {
            None => {
                ...
                if tick.price >= cur_grid + box_size {
                    ...
                    let mut top = cur_grid + box_size;
                    let mut bottom = top - box_size;
                    ...
                    let mut top = cur_grid;
                    let mut bottom = cur_grid - box_size;

...
// Crosshair 负责维护十字线的坐标、可见性以及价格/时间轴标签视图
export class Crosshair extends DataSource {
	...
	public constructor(model: IChartModelBase, options: CrosshairOptions) {
		// Crosshair 作为 DataSource 被 PaneWidget 绘制，与 model 共享 TimeScale、PriceScale
		super();
		this._model = model;
		this._options = options;

		// valuePriceProvider/valueTimeProvider 会根据不同 PriceScale/TimeScale 提供相应坐标与数值
	...
	public paneViews(pane: Pane): readonly IPaneView[] {
		// Pane 级别视图包含主十字线绘制与标记（marker）绘制，使用 WeakMap 缓存
	...
	public override priceAxisViews(pane: Pane, priceScale: PriceScale): IPriceAxisView[] {
		// 每次进入时确保只有当前 pane 的十字线标签存在，避免残留缓存
	...
	public updateAllViews(): void {
		// 十字线位置更新后需要刷新所有关联视图（pane、价格轴、时间轴）
	...
	private _priceScaleByPane(pane: Pane): PriceScale | null {
		// 默认使用 pane 的默认价格轴；若为空则十字线只显示时间标签
	...
	private _tryToUpdateData(newIndex: TimePointIndex, newPrice: number, newPane: Pane): boolean {
		// 根据传入的 index/price/pane 计算对应像素坐标，若任何值变化则返回 true
		const oldX = this._x;
		const oldY = this._y;
	...
	private _setIndexToLastSeriesBarIndex(): void {
		// 当需要重置十字线位置时，取所有 series 的最后索引作为 fallback

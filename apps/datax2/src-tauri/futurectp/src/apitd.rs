...
impl TraderSpi for BaseTraderSpi {
    fn on_front_connected(&mut self) {
        ...
        let td_user_id = self
            ...
            .expect("missing td_user_id in props");
        let td_app_id = self
            .config
            .get_attr("td_app_id")
            .and_then(|v| v.as_str().map(|s| s.to_string()))
            .expect("missing td_app_id in props");
        let td_auth_code = self
            .config
            .get_attr("td_auth_code")
            .and_then(|v| v.as_str().map(|s| s.to_string()))
            .expect("missing td_auth_code in props");
        req.UserID.assign_from_str(&td_user_id);
        req.AppID.assign_from_str(&td_app_id);
        req.AuthCode.assign_from_str(&td_auth_code);
    ...
    fn on_rsp_authenticate(
        ...
        if b_is_last {
            ...
            let td_user_id = self
                ...
                .and_then(|v| v.as_str().map(|s| s.to_string()))
                .expect("missing td_user_id in props");
            let td_password = self
                .config
                .get_attr("td_password")
                .and_then(|v| v.as_str().map(|s| s.to_string()))
                .expect("missing td_password in props");
            req.UserID.assign_from_str(&td_user_id);
            req.Password.assign_from_str(&td_password);
    ...
    fn on_rsp_user_login(
        ...
        if b_is_last {
            ...
            let td_user_id = self
                ...
                .and_then(|v| v.as_str().map(|s| s.to_string()))
                .expect("missing td_user_id in props");
            req.InvestorID.assign_from_str(&td_user_id);
...
pub fn create_td(config: CtpAccountConfig) -> Arc<TraderApi> {
    println!("tdapi create here!");
    let td_dynlib_path = config
        .get_attr("td_dynlib_path")
        .and_then(|v| v.as_str().map(|s| PathBuf::from(s)))
        .expect("missing td_dynlib_path in props");
    println!("td dynlib_path: {}", td_dynlib_path.to_string_lossy());

    #[cfg(not(feature = "ctp_v6_7_11"))]
    let tdapi = TraderApi::create_api(&td_dynlib_path, "./td_");

    #[cfg(feature = "ctp_v6_7_11")]
    let tdapi = TraderApi::create_api(&td_dynlib_path, "./td_", true);

    let tdapi = Arc::new(tdapi);

    // 先获取 front_address，避免 move 后的借用问题
    let front_address = config
        .get_attr("td_front_address")
        .and_then(|v| v.as_str().map(|s| s.to_string()))
        .expect("missing td_front_address in props");
